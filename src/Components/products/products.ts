...
@Component({
  ...
  imports: [NgClass,NgStyle,FocusContent,ScaleImage,ThemeChange,CurrencyPipe,DescriptionSlicePipe,TitleCasePipe,FormsModule],
...
increaseQuantity(product: any) {
    // لو الكمية مش متعرفة، نعتبرها 1
    if (!product.quantity) {
      product.quantity = 1;
    }
    // نزود الكمية بشرط إنها متعديش المخزون
  ...
  decreaseQuantity(product: any) {
    ...
    // ننقص الكمية بشرط إنها متقلش عن 1
...
prodfilter(){
  if(this.selectedCategory==="all"){
    ...
    this.filteredProducts=this.ProdList.filter(p=>p.category===this.selectedCategory);
  }
}
}
